          ...
          <tbody>
            {products.map((p: any) => (
              <tr key={p.id} className="border-b border-ash-gray-50 hover:bg-ash-gray-50/50 transition-colors">
                <td className="px-5 py-4"><div className="flex items-center gap-3.5"><div className="w-11 h-11 bg-ash-gray-100 rounded-lg flex items-center justify-center text-lg shrink-0">🏌️</div><div><p className="text-[13px] font-medium truncate max-w-[200px]">{p.name}</p><p className="text-[10px] text-ash-gray-400">{p.brand||"—"}</p></div></div></td>
